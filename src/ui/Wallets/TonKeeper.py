...
class TonKeeperWindow(PlantillaAirdrop):
    def __init__(self, window):
        super().__init__(title="Ton Keeper Wallet Creation")
        self.window = window
        ...
        # Añadir opciones específicas de Ton Keeper
    ...
    def add_execution_buttons(self):
        ...
        claim_button = QPushButton("Farm Ton Keeper")
    ...
    def button_farm_one(self):
        ...
        # Llamar a Ton Keeper Script con el archivo Excel
        self.farming_thread = iniciar_farmeo_multiple(selected_profiles, num_concurrent, "TonKeeper", window_settings)

        # Conectar la señal de finalización del hilo para modificar el Excel
        self.farming_thread.finished.connect(lambda: self.modificar_excel(archivo_excel))
    ...
    def modificar_excel(self, archivo_entrada):
        # Cargar el archivo Excel
        wb = load_workbook(archivo_entrada)
        ws = wb.active

        # Cambiar el fondo de la primera fila a gris medio oscuro
        gray_fill = PatternFill(start_color="A9A9A9", end_color="A9A9A9", fill_type="solid")
        for cell in ws[1]:
            cell.fill = gray_fill

        # Recorrer las filas a partir de la segunda
        for row in ws.iter_rows(min_row=2, max_row=ws.max_row):
            for cell in row:
                # Pintar de rojo las filas que contengan "ERROR"
                if cell.value == "ERROR":
                    for cell_in_row in row:
                        cell_in_row.fill = PatternFill(start_color="FF0000", end_color="FF0000", fill_type="solid")

            # Pintar de verde las celdas en la columna E que contengan "WALLET CREADA CON EXITO"
            if row[4].value == "WALLET CREADA CON EXITO":  # Columna E (índice 4)
                row[4].fill = PatternFill(start_color="00FF00", end_color="00FF00", fill_type="solid")
        ...
        wb.save(archivo_entrada)
        QMessageBox.information(self, "Éxito", "El archivo Excel ha sido modificado exitosamente.")
